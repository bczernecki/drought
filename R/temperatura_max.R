...
temperatura_map<- function(input="inp", output="outp"){
  ...
  breaks <-round(seq(-22, 41, length.out = length(tempcolores)),1)
  ...
  tm_shape(obj1) +
    tm_raster(title= paste0("Maksymalna dobowa temperatura powietrza [°C] \n", patt , " (00-23 UTC)"),
...
tmap_save(p + tm_layout(inner.margins = c(-0.1, -0.06, -0.1, -0.1)), 
          filename = paste0(www_path, "/t2m_max_",patt,".png"), width=1000, height=1300)

writeRaster(meantemp-273.15, filename = paste0(www_path,"/t2m_max_",patt,".tif"),overwrite=TRUE)