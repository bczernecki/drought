...
### poczatek rysowania:
setwd("/home/wh/tsk/czas_przymrozku_dobowo/")

rysuj = function(plik){
  input = raster(plik)
  patt = substr(plik, 1,10)
  p = temperatura_map(input=input, title = paste0("Czas przymrozku w ciągu doby (h)\n", patt))
...
for (i in 1998:2020){
  ...
  niewyszly = which(!file.exists(paste0("/home/wh/tsk/czas_przymrozku_dobowo_png/",gsub(x = files, "tif", "png"))))
