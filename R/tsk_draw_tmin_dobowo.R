...
temperatura_map<- function(input="inp", output="outp", title = "tytul"){
  ...
  tm_shape(obj1) +
    ...
    tm_shape(centroidy)+
    tm_text("column", size = 0.9, col = "white") +
...
rysuj = function(plik){
  ...
  p <- temperatura_map(input=input, title = paste0("Minimalna dobowa temperatura gruntu [°C] \n", patt , " (00-23 UTC)"))
  
  tmap_save(p, #+ tm_layout(inner.margins = c(-0.1, -0.06, -0.1, -0.1)), 
            filename = paste0("/home/wh/tsk/tmin_dobowe_png/", 
                              patt, "_tsk_tmin.png"), 
...
for (i in 1998:2020){
  ...
  niewyszly = which(!file.exists(paste0("/home/wh/tsk/tmin_dobowe_png//",gsub(x = files, "tif", "png"))))
