...
##########################  opad  #######################################
for (rok in 2018:1998){
  print(rok)
df1 <- data.frame(date = seq.Date(as.Date(paste0(rok, "-01-01")), as.Date(paste0(rok, "-12-31")), by="day"))
...
getdecade <- function(no = 4){
  ind <- which(df1$decade %in% no:(no-2))
...
#htc_maps <- pbmclapply(10:30, htc_decade, mc.cores = 21)
#htc_maps <- stack(htc_maps)
htc_maps2 <- pbmclapply(10:30, htc_decade2, mc.cores = 32)
htc_maps2 <- mclapply(htc_maps2, function(x){ x[x>3] <- 3; x}, mc.cores = 21) # szybka reklasyfikacja
htc_maps2 <- stack(htc_maps2)


#opadek <- pbmclapply(10:30, function(x) sumfunction(stack(paste0("data/",getdecade(no = x)$opadfiles))), mc.cores = 21)
#opadek <- stack(opadek)
...
# plotting htc:
# png(filename = paste0(rok, "htc_wrf.png"), width = 1201, height = 981, pointsize = 30)
# names(htc_maps) <- sapply(10:30, FUN = function(x) nazwy(x))
...
# print(p)
# dev.off()
# writeRaster(x = htc_maps, filename = paste0(rok, 'htc_wrf.tif'), overwrite =T)

# plotting htc2:
png(filename = paste0(rok, "htc_wrf_30days.png"), width = 1201, height = 981, pointsize = 30)
names(htc_maps2) <- sapply(10:30, FUN = function(x) nazwy(x))
#col = c("black","brown","red","orange","orange","yellow","skyblue","blue2","purple")
col <- data.frame(red = c(255, 255, 255, 127, 0, 0, 0, 0, 0),
                     green = c(0, 165, 214, 255, 255, 100, 0, 0, 0),
                     blue = c(0,0,0,0,0,0,255,127,0))


p=spplot(htc_maps2[[1:20]], zlim=c(0,3), 
         at=c(0, 0.3, 0.7, 1, 1.3, 1.6, 2, 2.5, 3), 
         col.regions=rgb(red = col$red, green = col$green, blue = col$blue, maxColorValue = 255), 
         layout =c(5,4))
p=p + layer(sp.polygons(pol, lwd=2.2, col='gray10',fill=F))
print(p)
dev.off()
writeRaster(x = htc_maps2, filename = paste0(rok, 'htc_wrf_30days.tif'), overwrite =T)

# plotting precipitation:
# png(filename = paste0(rok, "opad_wrf_v3.png"), width = 1201, height = 981, pointsize = 30)
# names(opadek) <- sapply(10:30, FUN = function(x) nazwy(x))
# opadek <- mask(opadek, pol)
# col = c("black","brown","red","pink","orange","yellow","yellowgreen", "skyblue","blue2","steelblue","purple")
# p=spplot(opadek[[1:20]], zlim=c(0,150), at=c(0,10,20,30,40,50,60,75, 100,150), col.regions=col, layout =c(5,4))
# p=p + layer(sp.polygons(pol, lwd=2.2, col='gray10',fill=F))
# print(p)
# dev.off()
# writeRaster(x = opadek, filename = paste0(rok, 'opad_wrf_v3.tif'), overwrite =T)

# observational precip:
#png(filename = paste0(rok ,"opad_imgw.png"), width = 1201, height = 981, pointsize = 30)
